...
def test_parameters_combinations(sweep_gen):
    test_sweep = copy.deepcopy(sweep_gen)
    test_sweep.set_parameter_values("probe", "labelling_efficiency", values=(0.1,1 ,3))
    test_sweep.set_parameter_values("particle_defect", "defect", values=(0.1,0.5,3))
    ...
    param_groups = list(sweep_gen.params_by_group.keys())
    ...
    for group_name in param_groups:
        if len(test_sweep.params_by_group[group_name]) > 0:
            for param_name in test_sweep.params_by_group[group_name]:
                total_combinations *= len(test_sweep.params_by_group[group_name][param_name])
