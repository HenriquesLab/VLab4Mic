...
def xmersubset_byclustering(
    ...
    """
    Model Structureal incomplete_labelling by nearest neighbors of the emitters and clustering
    ...
    if ids_validated is None:
        print("error while simulating incomplete_labelling, returning No emitters")
        if return_ids:
            default_false = [False] * epitopes_coords.shape[0]
            return default_false
        else:
