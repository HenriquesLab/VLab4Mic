...
def labeling_reorient_set(label_points, piv_id, intern_axis_id, direction, end_point):
    ...
    # intern_axis_id is an index for the other point in pts that will define its internal axix of rotation
    #   this also dictates the orientation of the internal axis so that 
    # direction sets the new direction all the points need to follow relative to its internal axis
    ...
    translated_origin, displacement = transform_displace_set(label_points, label_points[piv_id], np.array([0, 0, 0]))
    ...
    theta = np.arccos(np.dot(int_axis, direction) / (np.linalg.norm(int_axis) * np.linalg.norm(direction)))
    # Rotate pts  
    rotated_at_origin = rotate_set(translated_origin, V, theta)
    # take them back to the reference point by translating the points back to the reference point
    reoriented, _ = transform_displace_set(rotated_at_origin, rotated_at_origin[piv_id], end_point)
    return reoriented, V


def transform_displace_set(points, external_reference, end_point): # previously displace_set_3
    # points are the set of points to displace
    # external_reference is the reference point that usually is inside the cloud of points
    ...
    # transl_vec *= -1 ## this ensures that when the vector is added, it traslates automatically
    return translated, transl_vec


def rotate_pts_by_vector(pts, current_vector, new_vector, pts_referece_center):
    #rotated_pts_at_0, V = labeling_reorient_set(pts, pivot, intern_axis_id, new_direct, np.array([0,0,0]))
    translated_origin, displacement = transform_displace_set(pts, pts_referece_center, np.array([0, 0, 0]))
    ...
    # get angle or rotation in radians
    theta = np.arccos(np.dot(current_vector, new_vector) / (np.linalg.norm(current_vector) * np.linalg.norm(new_vector)))
    # Rotate pts  
    rotated = rotate_set(translated_origin, ax_of_rot, theta)
    # take them back to the reference point by translating the points back to the reference point
    reoriented, _ = transform_displace_set(rotated, np.array([0, 0, 0]), pts_referece_center)
    return reoriented
...
def decorate_epitopes_normals(normals_ft_epitopes, labeling_entity):
    ...
    for repl in range(np.shape(normals_ft_epitopes)[1]):
        new_points, rot_axis = labeling_reorient_set(labeling_entity,
...
def cleanup_labeling_entities(labels_on_epitopes):
    ...
    fluorophores_xyz = labels_on_epitopes[pivot_reference_index][2:] # the first two are to be removed
    for i in range(n_epitopes-1):
        # joining them like this allows for different number of emitters per antibody
        fluorophores_xyz = np.vstack((fluorophores_xyz, labels_on_epitopes[i+1][2:]))  # from i+1 since we already added the first
