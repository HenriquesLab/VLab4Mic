...
def sym_transforms_numeric_only(assembly_transf_dictionary, point_coords, by_sym_unit = False, show_transforms_used = False):
    # point coords is a numpy array with dimensions nx3, where n is the number of points to transform
    # assembly_transf_dictionary is a dicitonary where each key is the name of the transform,
    # and the value of each key a list. the first element of the list is the rotation matrix,
    ...
    for op in assembly_transf_dictionary.keys():
        ...
        if show_transforms_used:
            print(op, flag)
        if flag:
            new_points.append(do_sym_operation(point_coords, assembly_transf_dictionary[op]))
    new_points_per_sym = np.array(new_points)
    if by_sym_unit:
        return new_points_per_sym
    else:
        allatoms = new_points_per_sym.reshape((np.shape(new_points_per_sym)[0])*(np.shape(new_points_per_sym)[1]), 3)
        return allatoms


def summarize_epitope_atoms(atoms_per_epitope, method="average", residue_pos=0):
    # TODO: finish this funciton so that one can decide how to sumarize the coordintes from the epitopes
...
def array_coords_subset(array, percentage): #TODO: use sample array function from utils
    ...
    if percentage < 1:
        size = array.shape[0]
        ids = np.random.choice(np.arange(0, size), int(size*percentage), replace=False)
        subset = array[ids, :]
        return subset
    elif percentage == 1:
        return array
...
def create_instance_label(coords_normals: dict,
                        target_type: str,
                        label_data: dict,
                        **kwargs):
...
def indirect_labelling(coords_nomrals, label_data, **kwargs):
    ...
    if label_data["emitters"] is not None:
        ...
        epitopes, normals = binomial_epitope_sampling(coords_nomrals["coordinates"], efficiency, coords_nomrals["normals"]) #####################
        normals_ft_epitopes = [normals, epitopes]
        indirect_realisation = decorate_epitopes_normals(normals_ft_epitopes,
                                                    label_data["emitters"])
    else:
